...
RUN curl -s -L https://get.helm.sh/helm-v3.8.2-linux-arm64.tar.gz -o /tmp/helm-linux-amd64.tar.gz && \
	mkdir -p /tmp/helm && \
...
RUN chmod +x /entrypoint.sh /bin/drone-plugin && helm plugin install https://github.com/chartmuseum/helm-push
